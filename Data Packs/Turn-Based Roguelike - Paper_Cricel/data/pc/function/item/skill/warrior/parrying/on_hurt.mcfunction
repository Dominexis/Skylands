...
function pc:sys/utils/local_thread/atker/save {id:"skill.warrior.parrying"}

execute unless entity @s[tag=pc.local_thread.skill.warrior.parrying.atker] as @e[tag=pc.local_thread.skill.warrior.parrying.atker,limit=1,x=2048.0,y=-130.0,z=5120.0,dx=512.0,dy=450.0,dz=512.0,type=!minecraft:giant] at @s rotated ~180 ~ run function pc:sys/entity/repel/use

function pc:sys/utils/local_thread/atker/load {id:"skill.warrior.parrying"}

# fx
function pc:sys/fx/text/use {text:{nbt:"sys.fx.parrying",storage:"pc:lang",interpret:1b}}
...
playsound pc:sys/nine_sols_parry master @a ~ ~1024 ~ 0 1 1