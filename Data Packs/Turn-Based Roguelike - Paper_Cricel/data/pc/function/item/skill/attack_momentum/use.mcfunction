...
data modify storage pc:temp desc.lore set value {item:'attack_momentum', index:4, text:[{text:"",italic: false,color: "gray"},{text:"● ",color:"#72bd7a"},{text:"ᴛɪᴍᴇꜱ : "},{score:{name:"#var",objective:"pc.main"}}]}
function pc:sys/item/refresh/use
scoreboard players operation #temp.skill.attack_momentum pc.main = #var pc.main

execute as @e[type=marker,tag=pc.grid.now,x=2048.0,y=-130.0,z=5120.0,dx=512.0,dy=450.0,dz=512.0] at @s run function pc:item/skill/attack_momentum/2
...
playsound minecraft:entity.player.attack.sweep master @a[predicate=sl:player,x=2048.0,y=-130.0,z=5120.0,dx=512.0,dy=450.0,dz=512.0] ~ ~ ~ 1 1.25
