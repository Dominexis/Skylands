...
execute if data storage pc:temp item.ep.base run data modify storage pc:temp update.item_name.ep set value ["",{bold:true,color:"gold",nbt:"item.ep.value",storage:"pc:temp"},{"font":"pc:sys/icon",text:"\ue00b0 "}]
