...
execute unless data entity @s {data:{select:{avail:true}}} run scoreboard players set #pass pc.main 0
$execute positioned ~-1.5 ~-100 ~-1.5 if data entity @s data.occupy.$(object) unless entity @n[type=!player,tag=pc.grid.selector,dx=2,dy=200,dz=2] run scoreboard players set #pass pc.main 2
