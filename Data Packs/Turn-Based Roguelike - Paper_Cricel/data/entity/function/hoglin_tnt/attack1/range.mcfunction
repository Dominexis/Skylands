...
data modify storage pc:temp intent set value {damage:4,text:['damage','attack','walk'],tags:['pc.intent.walk']}
...
execute rotated as @s as @e[tag=pc.intent.now,tag=pc.intent.walk,tag=!pc.intent.potential,type=marker] positioned as @s run function sys:grid/range/use
