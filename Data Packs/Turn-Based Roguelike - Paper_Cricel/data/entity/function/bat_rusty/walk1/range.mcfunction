...
# if (intent == exist) :
execute at @n[tag=pc.intent.now,sort=furthest,type=marker,distance=..20] run return run function sys:grid/range/use

# else :
function sys:grid/range/use
execute at @s positioned ^ ^ ^4 positioned ~-1.5 ~-100 ~-1.5 run data merge entity @e[tag=pc.intent.now,dx=2,dy=200,dz=2,limit=1] {data:{is_walk:1b,text:['damage','attack','walk']}}