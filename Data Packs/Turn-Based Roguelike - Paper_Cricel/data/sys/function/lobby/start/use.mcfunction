...
# if (refuse) :
execute if entity @s[tag=!pc.lobby.character.chose] run return run function sys:lobby/start/refuse/no_char
execute if score $start pc.game matches 1 run return run function sys:lobby/start/refuse/started

# else :
function sys:lobby/end/clear

$function game:$(map)/start
...
function sys:lobby/start/hotbar
data modify storage pc:game stat.is_win set from storage pc:lang sys.lobby.end.death

function sys:lobby/start/player
function sys:combat/tpter/return/use

function sys:lobby/start/forceload with storage pc:game coord
schedule function sys:combat/time 1s
schedule function sys:lobby/start/2 2s replace
